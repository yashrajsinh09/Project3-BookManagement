...
const createBook = async (req, res) => {
    try {
        ...
        if (!isValidText(title)) res.status(400).send({ status: false, message: `${title} is not valid.` });
        const existsTitle = await bookModel.findOne({ title })
        if (existsTitle) return res.status(400).send({ status: false, message: `'${existsTitle}' this title already exists.` });
        ...
        const existsISBN = await internModel.findOne({ ISBN })
